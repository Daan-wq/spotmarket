...
export default async function SubmissionsPage() {
  ...
  return (
    <div className="p-8">
      ...
      <p className="mb-6" style={{ color: "var(--text-secondary)" }}>Review and approve submissions</p>
        ...
        <table className="w-full">
          <thead>
            <tr style={{ borderBottom: "1px solid var(--border)" }}>
              ...
              <th className="px-6 py-3 text-left text-xs font-semibold" style={{ color: "var(--text-secondary)" }}>Views</th>
          ...
          <tbody>
            {submissions.map((s) => (
              <tr key={s.id} style={{ borderBottom: "1px solid var(--border)" }}>
                ...
                <td className="px-6 py-3 text-sm"><SubmissionActions id={s.id} status={s.status} /></td>
              </tr>
            ))}
          </tbody>
        </table>
