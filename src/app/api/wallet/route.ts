...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      pendingReviewBalance: summary?.pendingReviewBalance ?? 0,
      lockedPayoutBalance: summary?.lockedPayoutBalance ?? 0,
      paidBalance: summary?.paidBalance ?? 0,
      profit: summary?.profit ?? 0,
      payoutIban: user.creatorProfile?.payoutIban ?? null,
      payoutAccountName: user.creatorProfile?.payoutAccountName ?? null,
      payoutSolanaAddress: user.creatorProfile?.payoutSolanaAddress ?? null,
      totalEarnings: summary?.totalEarned ?? 0,
      settledEarnings: summary?.paidBalance ?? 0,
      estimatedEarnings: 0,
