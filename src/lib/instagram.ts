...
export async function exchangeCodeForToken(
  ...
  if (!longRes.ok) {
    ...
    console.error("[instagram] long-lived exchange failed:", longErr);
...
export interface DailyAccountInsight {
  ...
  // Post counts per day (derived from media data, not insights API)
  reelsPosted: number;
 ...
 * Some metrics (views, accounts_engaged, total_interactions) only support
 * metric_type=total_value, so they are returned as window totals rather than per-day.
 */
export async function fetchAccountDailyInsights(
  ...
  // ── Call 1: per-day metrics (only reach + follower_count support daily breakdowns) ──
  ...
  if (!dailyRes.ok) {
    throw new Error(`fetchAccountDailyInsights (daily) failed: ${await dailyRes.text()}`);
  }
  const dailyData = await dailyRes.json();
  const core = extractDayValues((dailyData.data ?? []) as { name: string; values: { value: number; end_time: string }[] }[]);

  const dateKeys = Object.keys(core["reach"] ?? {}).sort();

  // ── Call 2: total_value metrics (these only return a single total for the window) ──
  let totalViews: number | null = null;
  let totalAccountsEngaged: number | null = null;
  let totalInteractionsVal: number | null = null;
  ...
  try {
    const totalParams = new URLSearchParams({
      metric: "views,accounts_engaged,total_interactions,likes,comments,saves,shares,reposts,replies",
    ...
    const totalRes = await fetch(`${base}?${totalParams}`);
    if (totalRes.ok) {
      const totalData = await totalRes.json();
      for (const item of (totalData.data ?? [])) {
        const val = item.total_value?.value ?? null;
        if (item.name === "views") totalViews = val;
        if (item.name === "accounts_engaged") totalAccountsEngaged = val;
        if (item.name === "total_interactions") totalInteractionsVal = val;
        if (item.name === "likes") totalLikes = val;
        if (item.name === "comments") totalComments = val;
        if (item.name === "saves") totalSaves = val;
        if (item.name === "shares") totalShares = val;
        if (item.name === "reposts") totalReposts = val;
        if (item.name === "replies") totalReplies = val;
      }
    }
  } catch {
    // Non-fatal
  ...
  // ── Call 3: follows_and_unfollows breakdown=follow_type ──
  ...
  // ── Call 4: profile_links_taps breakdown=contact_button_type ──
  ...
  // Build daily entries — only reach + follower_count have per-day breakdowns.
  // Post counts are merged in later from media data.
  const daily: DailyAccountInsight[] = dateKeys.map((dateKey) => ({
    date: dateKey,
  ...
  return {
    ...
    windowTotals: {
      views: totalViews ?? 0,
      accountsEngaged: totalAccountsEngaged ?? 0,
      totalInteractions: totalInteractionsVal ?? 0,
      likes: totalLikes ?? 0,
...
/**
 * Merge post-per-day counts from media items into daily insight entries.
 * Uses media_type (IMAGE, VIDEO, CAROUSEL_ALBUM) and media_product_type (REELS, STORY, FEED)
 * to classify each post, then increments the matching day's counter.
 */
export function mergeDailyPostCounts(
  daily: DailyAccountInsight[],
  media: { media_type: string; media_product_type: string; timestamp: string }[]
): DailyAccountInsight[] {
  const dayMap = new Map<string, DailyAccountInsight>();
  ...
  for (const m of media) {
    ...
    if (product === "REELS" || (product === "REEL")) {
      ...
      entry.carouselsPosted++;
    } else {
      // IMAGE or VIDEO on feed
      entry.imagesPosted++;
    }
  }
